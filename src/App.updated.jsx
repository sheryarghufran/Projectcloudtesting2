...
async function ai(prompt, tokens) {
  for (var i = 0; i < 3; i++) {
    try {
      ...
      if (!r.ok) {
        var msg = d && d.error ? d.error : "API failed";
        throw new Error(msg);
      }
...
export default function App() {
  ...
  var sendChat = useCallback(async function() {
    ...
    try {
      ...
      if (!r.ok) {
        var msg2 = d && d.error ? d.error : "Error. Try again.";
      ...
      setChat(function(p) { return p.concat([{ role: "assistant", content: e.message || "Error. Try again." }]); });
    }
    setChatLoad(false);
